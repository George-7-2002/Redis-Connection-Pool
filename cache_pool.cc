...
std::string CacheManager::conf_path_ = "tc_http_server.conf"; // default
...
/*
 * Initialize or reconnect to Redis, similar to mysql_ping()
 */
int CacheConn::Init() {
    if (context_) // Not null, connection is OK
    {
        return 0;
    }

    // Try to reconnect once per second
    uint64_t cur_time = (uint64_t)time(NULL);
    if (cur_time < last_connect_time_ + 1) // Reconnect attempt interval 1s
    {
    ...
    // Use redisContext to store connection state.
    // redisContext updates err and errstr fields after each operation
    // to indicate error code (>0) and description.
    context_ =
...
string CacheConn::Set(string key, string value) {
    ...
    // The result is stored in redisReply
    ...
    freeReplyObject(reply); // Free resource
    return ret_value;
}
...
int CacheConn::ZsetAdd(string key, long score, string member) {
    ...
    // Execute command, reply->integer returns 1 on success, 0 on failure
...
int CacheConn::ZsetZrevrange(string key, int from_pos, int end_pos,
    ...
    int count = end_pos - from_pos + 1; // Number of elements requested

    // Get elements from sorted set in descending order
    reply = (redisReply *)redisCommand(context_, "ZREVRANGE %s %d %d",
                                       key.c_str(), from_pos, end_pos);
    if (reply->type != REDIS_REPLY_ARRAY) // If return type is not array
    {
    ...
    // Returns an array, check reply->elements (array size)
    // Access array elements via element[index]
    // Each element is a pointer to a redisReply object

    max_count = (reply->elements > count) ? count : reply->elements;
    get_num = max_count; // Number of values actually retrieved

    for (i = 0; i < max_count; ++i) {
        strncpy(values[i], reply->element[i]->str, VALUES_ID_SIZE - 1);
        values[i][VALUES_ID_SIZE - 1] = 0; // Null terminator
    }
...
CacheConn *CachePool::GetCacheConn(const int timeout_ms) {
    ...
    if (free_list_.empty()) // 2. When no connection is available
    {
        // First, check if current connection count has reached max
        if (cur_conn_cnt_ >= max_conn_cnt_) // Wait logic
        {
            // If max is reached, check if we need to wait with a timeout
            if (timeout_ms <= 0) // Wait indefinitely until a connection is available or pool is aborted
            {
                log_info("wait ms:%d\n", timeout_ms);
                cond_var_.wait(lock, [this] {
                    // Exit when free_list_ is not empty or abort_request_ is true
                    return (!free_list_.empty()) | abort_request_;
                });
            } else {
                // If lambda returns false, continue waiting (or timeout)
                // If lambda returns true, exit wait. Reasons:
                // 1. free_list_ is not empty
                // 2. Timeout (wait_for returns)
                // 3. abort_request_ is true
                cond_var_.wait_for(
                    lock, std::chrono::milliseconds(timeout_ms),
                    [this] { return (!free_list_.empty()) | abort_request_; });
                
                // With timeout, must check if list is still empty after wait
                if (free_list_.empty()) // If pool is still empty, exit
                {
                    return NULL;
                }
            }
        ...
        } else // Not at max, create a new connection
        {
            CacheConn *db_conn =
                new CacheConn(server_ip_.c_str(), m_server_port, db_index_,
                              password_.c_str(), pool_name_.c_str()); // Create new connection
...
int CacheManager::Init() {
    ...
    if (!cache_instances) {
        LogError("CacheInstances not configured");
        return 1;
    }
    ...
    for (uint32_t i = 0; i < instances_name.GetItemCnt(); i++) {
        ...
        if (!cache_host || !str_cache_port || !str_cache_db ||
            !str_max_conn_cnt) {
            if(!cache_host)
                LogError("not configure cache instance: %s, cache_host is null", pool_name);
            if(!str_cache_port)
                LogError("not configure cache instance: %s, str_cache_port is null", pool_name);
            if(!str_cache_db)
                LogError("not configure cache instance: %s, str_cache_db is null", pool_name);
            if(!str_max_conn_cnt)
                LogError("not configure cache instance: %s, str_max_conn_cnt is null", pool_name);
            return 2;
        }
