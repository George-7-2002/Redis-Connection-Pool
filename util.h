#ifndef __UTIL_H__
#define __UTIL_H__

#define _CRT_SECURE_NO_DEPRECATE // remove warning C4996,

#include "lock.h"
#include "ostype.h"
#include "util_pdu.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#ifndef _WIN32
#include <strings.h>
#endif

#include <assert.h>
#include <sys/stat.h>

#ifdef _WIN32
#define snprintf sprintf_s
#else
#include <pthread.h>
#include <stdarg.h>
#include <sys/time.h>
#include <time.h>
#endif

#define NOTUSED_ARG(v)                                                         \
    ((void)v) // used this to remove warning C4100, unreferenced parameter

/// yunfan modify end

/**
 * @brief A base class for reference-counted objects.
 */
class CRefObject {
  public:
    CRefObject();
    virtual ~CRefObject();

    /**
     * @brief Sets the lock to be used for thread-safe ref counting.
     */
    void SetLock(CLock *lock) { lock_ = lock; }
    
    /**
     * @brief Increments the reference count (thread-safe if lock is set).
     */
    void AddRef();
    
    /**
     * @brief Decrements the reference count. Deletes the object if count reaches zero.
     */
    void ReleaseRef();

  private:
    int ref_count_;
    CLock *lock_;
};

/**
 * @brief Gets the number of milliseconds elapsed since the system started.
 */
uint64_t GetTickCount();

/**
 * @brief Platform-independent sleep function.
 * @param millisecond The number of milliseconds to sleep.
 */
void util_sleep(uint32_t millisecond);

/**
 * @brief A utility class to split a C-string (in-place) by a separator.
 */
class CStrExplode {
  public:
    /**
     * @brief Constructs and splits the string.
     * @param str The input string (will be modified).
     * @param seperator The delimiter character.
     */
    CStrExplode(char *str, char seperator);
    
    /**
     * @brief Frees the allocated item list.
     */
    virtual ~CStrExplode();

    /**
     * @brief Returns the number of items found after splitting.
     */
    uint32_t GetItemCnt() { return item_cnt_; }
    
    /**
     * @brief Returns the item at the specified index.
     */
    char *GetItem(uint32_t idx) { return item_list_[idx]; }

  private:
    uint32_t item_cnt_;
    char **item_list_;
};

/**
 * @brief Replaces all occurrences of 'old_char' with 'new_char' in 'src' (in-place).
 */
char *ReplaceStr(char *src, char old_char, char new_char);


string Int2String(uint32_t user_id);


uint32_t String2Int(const string &value);


void ReplaceMark(string &str, string &new_value, uint32_t &begin_pos);

//Replaces the next '?' placeholder in the string with the given integer value.

void ReplaceMark(string &str, uint32_t new_value, uint32_t &begin_pos);

//Writes the current process ID to a .pid file.
void WritePid();

inline unsigned char ToHex(const unsigned char &x);
inline unsigned char FromHex(const unsigned char &x);

//Encodes a string for use in a URL.
string URLEncode(const string &sIn);

//Decodes a URL-encoded string.
string URLDecode(const string &sIn);


int64_t GetFileSize(const char *path);


const char *MemFind(const char *src_str, size_t src_len, const char *sub_str,
                    size_t sub_len, bool flag = true);

#endif
